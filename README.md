...
**Protect the effective encryption key as well as the database.** The first nonempty source wins: `DATA_ENCRYPTION_KEY`, then the `FLASK_SECRET_KEY` environment variable, then `secrets/data_encryption_key.txt`. Backing up only the fallback file is insufficient when an environment variable supplied the key. Preserve the relevant deployment configuration and any available historical keys needed by retained snapshots in private recovery storage accessible after losing the original host. Do not print keys in diagnostic output, commit them, or encrypt their only recovery copy with the same application key being protected.
...
1. Agree a maximum acceptable source-snapshot age in seconds, retention and recovery-time target for the backup service. Record the snapshot identity and original source-file timestamp separately from verified transfer time. Preserve that timestamp through staging, or record it in a verified manifest: filenames can be advanced to maintain rotation order and upload time does not establish snapshot freshness. A snapshot is stale when its age exceeds the agreed limit; repeatedly transferring an old snapshot must not reset its age.
2. Retrieve a completed snapshot, required configuration and recovery keys using access that survives loss of the original host. Restore into a temporary location with application workers, network access and email disabled; keep the original snapshot untouched. Optional `Media/` restoration avoids image re-downloads.
3. Run `PRAGMA integrity_check` and compare representative user, track and play counts with the protected snapshot's recorded counts. Inventory raw encrypted fields from `users` (`cookies_json`, `spotify_client_secret`, `spotify_refresh_token`, `lastfm_api_key`) and the `smtp_password` setting. For each available key, record readable, unreadable and intentionally unset counts without displaying plaintext or ciphertext. Pre-fingerprint values require an actual decryption attempt.
4. Record the drill date, snapshot identity/time, verified transfer time, key coverage, exceptions and achieved recovery time. Finding several necessary keys does not enable runtime multi-key support: unresolved credentials need a separate reassociation/re-entry or re-encryption procedure. Local snapshot success and synthetic tests alone do not establish off-machine recovery.
