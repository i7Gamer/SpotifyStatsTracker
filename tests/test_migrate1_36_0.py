...
class TestMigrate1_36_0(unittest.TestCase):
    ...
    migration chain still needs a step for every consecutive minor version, so
    this one exists purely to advance the marker - it must NOT touch the schema,
    and it must reject a database that isn't actually on 1.36.0."""
    ...
    def setUp(self):
        ...
        (self.root / "Database" / "VERSION").write_text("1.37.0", encoding="utf-8")
        (self.dataDir / "VERSION").write_text("1.36.0", encoding="utf-8")
    ...
    def test_bumps_the_version_marker(self):
        self._seedDatabaseAt("1.36.0")

        migrateModule.Migrator("1.36.0", "1.37.0").migrate()

        self.assertEqual((self.dataDir / "VERSION").read_text(encoding="utf-8").strip(), "1.37.0")
        self.assertEqual(dbversion.readDbVersion(self.dbPath), "1.37.0")
    ...
    def test_rejects_a_database_not_on_the_from_version(self):
        self._seedDatabaseAt("1.35.0")   #< not 1.36.0
    ...
    def test_migration_is_idempotent_on_retry(self):
        ...
        (self.dataDir / "VERSION").write_text("1.36.0", encoding="utf-8")   #< simulate a retry
        dbversion.writeDbVersion(self.dbPath, "1.36.0")
        migrateModule.Migrator("1.36.0", "1.37.0").migrate()   #< must not raise
