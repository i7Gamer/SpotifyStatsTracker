...
class TestMigrate1_14_0(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / "Database" / "VERSION").write_text("1.15.0", encoding="utf-8")
        (self.dataDir / "VERSION").write_text("1.14.0", encoding="utf-8")
    ...
    def test_creates_user_shares_table_and_bumps_version(self):
        ...
        with conn:
            ...
            conn.execute("INSERT INTO users (username, created_at) VALUES ('bob', 0)")
        ...
        self.assertIn("user_shares", tableNames)
        ...
        with conn:
            conn.execute(
                ...
                "VALUES ('alice', 'bob', 'pending', 0)"
            )
        row = conn.execute("SELECT status FROM user_shares WHERE requester_username='alice' AND recipient_username='bob'").fetchone()
    ...
    def test_migration_ddl_creates_the_table_without_schema_help(self):
        ...
        executescript(SCHEMA), which already contains user_shares, so the
        table exists before the migrator's CREATE TABLE runs. Here SCHEMA is
        patched back to its pre-user_shares form (everything before the
        table's section comment) so only the migration's own DDL can create
        it - a broken/renamed statement in migrate1_14_0.py fails this test."""
        preShareSchema = dbModule.SCHEMA.split("-- Mutual data-sharing")[0]
        self.assertNotIn("user_shares", preShareSchema)   #< the split really removed it

        with patch.object(dbModule, "SCHEMA", preShareSchema):
            migrateModule.Migrator("1.14.0", "1.15.0").migrate()
        ...
        self.assertIn("user_shares", names)
        self.assertIn("idx_user_shares_recipient", names)
        self.assertIn("idx_user_shares_requester", names)
