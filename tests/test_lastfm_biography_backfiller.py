...
class RunEventThreadingTestCase(BiographyWorkerBase):
    ...
    def test_the_loop_hands_its_private_event_to_every_batch(self, mockClientClass):
        ...
        with patch.object(db, "_processLastfmBiographyBatch", side_effect=capture):
            db._lastfmBiographyBackfillLoop(privateEvent)
    ...
    def test_a_set_run_event_stops_the_batch_despite_a_fresh_attribute(self):
        db = self._makeDbWithPlays()
        self.assertTrue(db.repo.getArtistsMissingBiographies(10, "user1"))   #< real rows to walk
        oldEvent = threading.Event()
        oldEvent.set()                                        #< this run was stopped
        db.lastfm_biography_stop_event = threading.Event()    #< the restart's fresh, unset event
        client = MagicMock()

        processed = db._processLastfmBiographyBatch(client, "user1", stop_event=oldEvent)
